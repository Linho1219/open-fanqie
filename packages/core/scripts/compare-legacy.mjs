...
const cases = [
  ...
  ...[24, 28, 32, 36, 42].map((biaoti_size) => [
    `title-size:${biaoti_size}`,
    { biaoti_size: String(biaoti_size) },
  ]),
  ...[16, 18, 22, 24, 28].map((fubiaoti_size) => [
    `subtitle-size:${fubiaoti_size}`,
    { fubiaoti_size: String(fubiaoti_size) },
  ]),
  ...[14, 16, 18, 20, 22, 24, 26, 28, 30, 32].map((geci_size) => [
    `lyric-size:${geci_size}`,
    { geci_size: String(geci_size) },
  ]),
...
function firstDifference(left, right) {
  ...
  for (let index = 0; index < length; index += 1) {
    if (stable(left[index]) !== stable(right[index]))
      return { index, api: left[index], local: right[index] }
  }
  return undefined
}
...
function firstUnorderedAttributeDifference(left, right) {
  const sorted = (items) =>
    items
      .map((item) => ({ item, comparable: comparableAttributes(item) }))
      .sort((a, b) =>
        stable(a.comparable).localeCompare(stable(b.comparable), undefined, { numeric: true }),
      )
...
function useMismatchContexts(mismatches, uses) {
  const notes = uses.filter(
    (item) => item.notepos !== undefined && /#shuzi_/.test(item['xlink:href'] ?? ''),
  )
  return mismatches.map((item) => ({
    item,
    nearest: [...notes].sort(
      (left, right) =>
        Math.abs(Number(left.x) - Number(item.x)) +
        Math.abs(Number(left.y) - Number(item.y)) -
        Math.abs(Number(right.x) - Number(item.x)) -
        Math.abs(Number(right.y) - Number(item.y)),
    )[0],
  }))
}
...
function firstPositionedUseDifference(left, right) {
  const byPosition = (items) =>
    items
      .filter((item) => item.notepos !== undefined)
...
function firstPositionedCoordinateDifference(left, right) {
  ...
  for (let index = 0; index < length; index += 1) {
    ...
    if (
      expected === undefined ||
      actual === undefined ||
      expected.notepos !== actual.notepos ||
...
function lineDetails(lines) {
  const beamsByY = {}
  lines
    .filter((line) => line['data-type'] === 'jianshixian')
    .forEach((line) => {
  ...
  return {
    ...
    other: lines.filter(
      (line) =>
        line['data-type'] === undefined && !(line['stroke-width'] === '4' && line.x1 === line.x2),
    ),
  }
}
...
function firstLinePositions(uses) {
  const positioned = uses
    .filter((item) => /^\d+_1_\d+$/.test(item.notepos ?? ''))
    .sort(
      (left, right) =>
        Number((left.notepos ?? '').split('_')[2]) - Number((right.notepos ?? '').split('_')[2]),
    )
  ...
  return {
    ...
    uses: {
      ...
      missing: missingSignatures(
        apiUses,
        localUses,
        (item) => `${item.notepos ?? ''}|${item['xlink:href'] ?? ''}|${item.code ?? ''}`,
      ).slice(0, 12),
      hrefs,
    },
    texts: {
